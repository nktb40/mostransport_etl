...
class Postgres:
	...
	def upload_stations(self):
		print("Start upload_stations")

		# Маркируем текущие записи как удалённые
		self.send_query("update stations set deleted_flag = true where city_id="+str(self.city_id)+"; commit;")

		# Составляем запрос
		q = "INSERT INTO stations (source_id,latitude,longitude,route_numbers,station_name,geo_data,city_id,created_at,updated_at,deleted_flag)"
		q += "\nselect source_id, latitude, longitude, route_numbers, station_name, cast(geo_data as JSON) as geo_data, "+str(self.city_id)+" as city_id, NOW(), NOW(), false as deleted_flag from ("
		...
		q += ("\nDO UPDATE SET "+
			  "latitude=EXCLUDED.latitude, longitude=EXCLUDED.longitude, route_numbers=EXCLUDED.route_numbers, station_name=EXCLUDED.station_name, geo_data=EXCLUDED.geo_data, updated_at=NOW(), deleted_flag=false;")
		q += "\ncommit;"
	...
	def upload_routes(self):
		...
		# Маркируем текущие записи как удалённые
		self.send_query("update routes set deleted_flag = true where city_id="+str(self.city_id)+"; commit;")

		# Составляем запрос
		q = "INSERT INTO routes (route_number,route_code,route_name,type_of_transport,geo_data,source_id,circular_flag,bbox,city_id,created_at,updated_at,deleted_flag)"
		q += "\nselect route_number,route_code,route_name,type_of_transport,cast(geo_data as JSON) as geo_data,source_id,cast(circular_flag as boolean) as circular_flag, cast(bbox as JSON) as bbox, "+str(self.city_id)+" as city_id, NOW(), NOW(),false from ("
		...
		q += "\nON CONFLICT (source_id,city_id)"
		q += ("\nDO UPDATE SET "+
			  "route_number=EXCLUDED.route_number, route_code=EXCLUDED.route_code, route_name=EXCLUDED.route_name, type_of_transport=EXCLUDED.type_of_transport, geo_data=EXCLUDED.geo_data, circular_flag=EXCLUDED.circular_flag, bbox=EXCLUDED.bbox, updated_at=NOW(), deleted_flag = false;")
		q += "\ncommit;"
	...
	def upload_lnk_station_routes(self):
		print("Start upload_lnk_station_routes")

		# Маркируем текущие записи как удалённые
		self.send_query("update lnk_station_routes set deleted_flag = true where station_id in (select id from stations where city_id="+str(self.city_id)+"); commit;")
		self.send_query("update lnk_station_routes set deleted_flag = true where route_id in (select id from routes where city_id="+str(self.city_id)+"); commit;")
		...
		for chunk in numpy.array_split(rows,math.ceil(len(rows)/1000)):
			# Составляем запрос
			q = "INSERT INTO lnk_station_routes (station_id,route_id,route_type,track_no,seq_no,route_time,distance,created_at,updated_at,deleted_flag)"
			q += "\nselect s.id as station_id, r.id as route_id, r2s.route_type, r2s.track_no, r2s.seq_no, r2s.route_time, r2s.distance, NOW(),NOW(), false from ("
			...
			q = q.rstrip(" union all")
			q += "\n) r2s" 
			q += "\nINNER JOIN stations s ON s.source_id = r2s.station_id AND s.city_id = "+str(self.city_id)
			q += "\nINNER JOIN routes r ON r.source_id = r2s.route_id AND r.city_id = "+str(self.city_id)
			q += "\nON CONFLICT (station_id,route_id,track_no)"
			q += ("\nDO UPDATE SET "+
				  "route_type=EXCLUDED.route_type, seq_no=EXCLUDED.seq_no, route_time=EXCLUDED.route_time, distance=EXCLUDED.distance, updated_at=NOW(), deleted_flag=false;")
			q += "\ncommit;"
	...
	def upload_isochrones(self, profile):
		print("Start upload_isochrones",profile)

		# Маркируем текущие записи как удалённые
		self.send_query("update isochrones set deleted_flag = true where city_id="+str(self.city_id)+" and profile='"+profile+"'; commit;")
		...
		for path in paths:
			...
			# Составляем запрос
			q = "INSERT INTO isochrones (station_id,source_station_id,route_id,source_route_id,unique_code,contour,profile,with_interval,with_changes,geo_data,properties,city_id,created_at,updated_at,deleted_flag)"
			...
			q += ("\nselect s.id as station_id, src.source_station_id, r.id as route_id, src.source_route_id, src.unique_code, "+
				  ...
				  "cast(src.properties as JSON) as properties,"+str(self.city_id)+" as city_id, NOW(),NOW(),false from (")
			...
			q = q.rstrip(" union all")
			q += "\n) src" 
			q += "\nLEFT JOIN stations s ON s.source_id = src.source_station_id AND s.city_id = "+str(self.city_id)
			q += "\nLEFT JOIN routes r ON r.source_id = src.source_route_id AND r.city_id = "+str(self.city_id)
			...
			q += ("\nDO UPDATE SET "+
				  ...
				  "properties=EXCLUDED.properties,city_id=EXCLUDED.city_id, updated_at=NOW(),deleted_flag=false;")
			q += "\ncommit;"
	...
	def upload_metrics(self,profile):
		print("Start upload_metrics",profile)

		# Маркируем текущие записи как удалённые
		self.send_query("update metrics set deleted_flag = true where isochrone_id in (select id from isochrones where city_id="+str(self.city_id)+" and profile='"+profile+"'); commit;")
		...
		for path in paths:
			...
			for chunk in numpy.array_split(rows,math.ceil(len(rows)/1000)):
				# Составляем запрос
				q = "INSERT INTO metrics (metric_type_id,isochrone_id,isochrone_unique_code,metric_value,created_at,updated_at,deleted_flag)"
				q += "\nselect m.id as metric_type_id, i.id as isochrone_id, src.isochrone_unique_code, src.metric_value, NOW(),NOW(),false from ("
				...
				q += ("\nDO UPDATE SET "+
					  "metric_type_id=EXCLUDED.metric_type_id,isochrone_id=EXCLUDED.isochrone_id, isochrone_unique_code=EXCLUDED.isochrone_unique_code, metric_value=EXCLUDED.metric_value, updated_at=NOW(), deleted_flag=false;")
				q += "\ncommit;"
				#print(q)
				# Отправялем запрос на вставку
				self.send_query(q)
	...
	def upload_station_metrics(self):
		print("Start upload_station_metrics")

		# Маркируем текущие записи как удалённые
		self.send_query("update station_metrics set deleted_flag = true where station_id in (select id from stations where city_id="+str(self.city_id)+"); commit;")

		# Составляем запрос
		q = "INSERT INTO station_metrics (metric_type_id,station_id,metric_value,created_at,updated_at,deleted_flag)"
		q += "\nselect m.id as metric_type_id, s.id as station_id, src.metric_value, NOW(),NOW(),false from ("
		...
		q += ("\nDO UPDATE SET "+
			  "metric_type_id=EXCLUDED.metric_type_id,station_id=EXCLUDED.station_id, metric_value=EXCLUDED.metric_value, updated_at=NOW(),deleted_flag=false;")
		q += "\ncommit;"
	...
	def upload_route_metrics(self):
		print("Start upload_route_metrics")

		# Маркируем текущие записи как удалённые
		self.send_query("update route_metrics set deleted_flag = true where route_id in (select id from routes where city_id="+str(self.city_id)+"); commit;")

		q = "INSERT INTO route_metrics (metric_type_id,route_id,metric_value,created_at,updated_at,deleted_flag)"
		q += "\nselect m.id as metric_type_id, r.id as route_id, src.metric_value, NOW(),NOW(),false from ("
		...
		q += ("\nDO UPDATE SET "+
			  "metric_type_id=EXCLUDED.metric_type_id, route_id=EXCLUDED.route_id, metric_value=EXCLUDED.metric_value, updated_at=NOW(), deleted_flag=false;")
		q += "\ncommit;"
	...
	def upload_city_metrics(self):
		print("Start upload_city_metrics")

		# Маркируем текущие записи как удалённые
		self.send_query("update city_metrics set deleted_flag = true where city_id ="+str(self.city_id)+"; commit;")

		q = "INSERT INTO city_metrics (metric_type_id,city_id,metric_value,created_at,updated_at,deleted_flag)"
		q += "\nselect m.id as metric_type_id, c.id as city_id, src.metric_value, NOW(),NOW(),false from ("
		...
		q += ("\nDO UPDATE SET "+
			  "metric_type_id=EXCLUDED.metric_type_id, city_id=EXCLUDED.city_id, metric_value=EXCLUDED.metric_value, updated_at=NOW(), deleted_flag=false;")
		q += "\ncommit;"
	...
	def upload_houses(self):
		print("Start upload_houses",self.city_code)

		# Маркируем текущие записи как удалённые
		self.send_query("update houses set deleted_flag = true where city_id ="+str(self.city_id)+"; commit;")
		...
		q = ("INSERT INTO houses(city_id,source_id,street_name,house_number,building,"+
			 ...
			 "area_total,area_residential,population,geometry,far_from_stops_flag,created_at,updated_at,deleted_flag) VALUES")
		...
		for house in houses:
			...
			q += ("\n("+str(self.city_id)+",'"+h['source_id']+"','"+h['street_name']+"','"+h['house_number']+"','"+h['building']+"','"+
				  h['block']+"','"+h['letter']+"','"+h['address']+"',"+floor_count_min+","+floor_count_max+","+
				  entrance_count+","+area_total+","+area_residential+","+str(h['population'])+",'"+geometry+"',"+far_flag+",NOW(),NOW(),false),")
		...
		q += ("\nDO UPDATE SET "+
			 	...
			 	"area_residential=EXCLUDED.area_residential,population=EXCLUDED.population,geometry=EXCLUDED.geometry,far_from_stops_flag=EXCLUDED.far_from_stops_flag,updated_at=NOW(),deleted_flag=false;")

		q += "\ncommit;"
		
		# Отправялем запрос на вставку
		self.send_query(q)
		print("Finish upload_houses",self.city_code)

	# Функция добавления нового слоя для города
	def upload_layer(self,layer_type_code):
		print("Start upload_layer",self.city_code,layer_type_code)

		# Получаем ID типа слоя в БД
		layer_type_id = send_query("select id from layer_types where source_name='"+layer_type_code+"'")[0][0]

		# Составляем запрос 
		q = ("INSERT INTO layers(city_id,layer_type_id,tile_url,created_at,updated_at) VALUES"+
			"("+str(self.city_id)+","+str(layer_type_id)+", 'nktb."+self.city_code+"-"+layer_type_code+"', NOW(),NOW())")
		...
		# Отправялем запрос на вставку
		send_query(q)
		print("Finish upload_layer",self.city_code,layer_type_code)
...
if __name__ == '__main__':
	...
	#upload_route_metrics()
	#upload_metrics("route_cover")
	#upload_lnk_station_routes('NNG')

	#p.upload_city()
	#p.upload_stations()
	#p.upload_routes()
	#p.upload_lnk_station_routes()
	#p.upload_isochrones("walking")
	#p.upload_metrics("walking")
	#p.upload_station_metrics()
	#p.upload_route_metrics()
	#p.upload_city_metrics()
	#p.upload_houses()